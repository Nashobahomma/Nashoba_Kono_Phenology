...
const r=2; {r=the max. number of traits}
      ...
      f=15;  {the max number of families}
      sibs=3247; {max # of individuals in a family, including unmeasured}
      rhsibs=3247;
      rfsibs=3247; {max # of recip full sibs in a family}
      pos=3247; {largest number of observations in a family (=< r*sibs)}
      nmax=3000; {maximum total # of individuals in the dataset, including
                 those missing data}
      missing=-98; {Value in the dataset indicating missing phenotypic data}
      epsilon = 0.001;  {Convergence criterion}
      maxround=20;
      nf = 3;  {the max. number of fixed effects}
      p = 37;  {the maximum number of levels of fixed effects=r~nlevels}
...
var  PresentData: Dataset;
     ...
     aset: active;
     cfactors: itriplet;
     LogLike, OldLogLike, kin:real;
     XVY,WtdVals, Means: Parammean;
     AddCov, DomCov, EnvCov:Paramvar;
     D, SumD, Parms:Parmvec;
     eD : eParmvec;
     Zeroed : Parmbool;
     better, alldone, llpos, restart, trouv: boolean;
     Done, REML, ok, Maxed, Trunked: boolean;
     bfl, feasible : boolean;
     flagvec: slevelsbool;
     sibships: text;
     result,Analysis: text;
     nchar, ntrunc, slevels, nfxfact, firstnum, temp :integer;
     newfam, I, J, I1, Iter, ncut, tncut: integer;
     Fm: Famarray;
     AB, VInvX: Famphens;
     Py: FamVector;
...
procedure order (var Data:Dataset; var me: integer; var nextnum: integer);

{assigns an ordering to the pedigree so that the matrix
coefficient stuff will work}

begin
if (Data[me].GEN = missing)
  then begin
{First get parental numbers if they are missing}
  if (Data[me].Father > 0)
     then 
     if (Data[Data[me].Father].GEN = missing)
     then order (Data, Data[me].Father, nextnum);
  if (Data[me].Mother > 0)
     then 
     if (Data[Data[me].Mother].GEN = missing)
     then order (Data, Data[me].Mother, nextnum);
{Parents now have numbers}
   Data[me].GEN := nextnum;
   nextnum := nextnum + 1;
end; {no need to do anything if Data[me].gen isnt missing}
end;
...
procedure PrintFams (var Data: Dataset; Zmatrix:Measures);
   ...
   var I, J: integer;
   begin
  writeln (Analysis, 'Loc ID   ID   GEN   Father Mother  FO     NMS    NPS  Famid Observations  ');
      for I := 1 to M do begin
         with Data[I] do begin
            write (Analysis, I:6, ID:6, GEN:6, Father:7, Mother:7, FO:7, NMS:7, NPS:7);
            write (Analysis, Famid:5);
            end;
            for J:= 1 to nchar do
...
procedure Collecthem (var Data: Dataset; var Fm: Famarray; var famsize: Size);
   ...
   begin {Collecthem}
      for I := 1 to famnum do begin
         ...
         famsize[I] := L-1;
         writeln("Family ",I:5," has",famsize[I]:5);
         end;
       end; {Collecthem}

procedure phi2 (atemp, btemp: integer; var ph2: real);
...
begin
 ...
 if ((a=0) or (b=0))
  ...
  then begin
  phi2(PresentData[a].Father,PresentData[a].Mother,kin1);
  ph2 := (1+kin1)/2;
  end
   else begin
      if (PresentData[a].GEN<PresentData[b].GEN) then begin
       { permute a and b}
       temp := b;
       b := a;
       a := temp;
       end;
     phi2(PresentData[a].Father,b,kin1);
     phi2(PresentData[a].Mother,b,kin2);
     ph2 := (kin1 + kin2)/2;
 end;
end;
{ here ends phi2 }

procedure Dom (var Ped: Dataset; var Fi: Famset; 
               x, y: integer; 
               var coeff: triplet);

 var I,J,mom1,mom2,pop1,pop2: integer; 
     kin1, kin2, kin3, kin4: real; 
...
begin {Dom procedure}
  ...
  I := Fi[x];
  J := Fi[y];
  if I=J then coeff[3] := 1.0;
  if ((Ped[I].Mother = 0) or (Ped[I].Father = 0) or 
       (Ped[J].Mother = 0) or (Ped[J].Father = 0) )
   then coeff[2] := 0.0 {no dom relationship between founders and observed}
   else
   if not (I = J)
   then begin {all four parents are present in pedigree}
    mom1 := Ped[I].Mother;
    mom2 := Ped[J].Mother;
    pop1 := Ped[I].Father;
    pop2 := Ped[J].Father;
    if ceo > 1 then begin {common env option}
     if ceo = 2 then begin
      if (mom1=mom2) then 
       coeff[2] := 1.0;
     end
     else if ceo = 3 then 
      if (mom1=mom2) and (pop1=pop2) then 
       coeff[2] := 1.0;
    end
    else begin {not ceo}
     phi2(mom1,mom2,kin1);
     ...
     phi2(mom1,pop2,kin3);
     phi2(pop1,mom2,kin4);
     coeff[2] := kin1*kin2 + kin3*kin4;
    end;
   end 
   else coeff[2] := 1.0; {every time i=j}
  {..and now the additive}
   phi2(I,J,kin1); {pick this up even if I=J}
   coeff[1] := kin1;
   if ((I=J) and (kin1 > 1.0)) then begin
    writeln ('Warning: Individual',I:8,' is inbred');
    end;
   coeff[1] := 2*coeff[1];
end;  {Dom procedure}
                  ...
                  for J:= 1 to famsize[G] do begin
                     for L:= 1 to nchar do begin
                        ...
                        then skippedj := skippedj + 1
                           ...
                           Dom(PresentData,Fm[G],I,J,coeff);
...
procedure Report;
   ...
   for i := 1 to totvar do begin
       ...
       writeln (result,Parms[1]:14:6,Parms[2]:14:6,B[1,1]:14:6,B[2,2]:14:6);
    end;
...
{order the pedigree for general descent measure algorithm}
   firstnum := 1;
   for I := 1 to M do begin
   temp := I;
   order (PresentData,temp,firstnum);
   end;
   ...
   repeat
    if not restart then begin
	 ...
	 writeln ('done comploglike');
      ...
      if not Done
      then begin
      writeln ('entering Calcnew with ncut =',ncut);
